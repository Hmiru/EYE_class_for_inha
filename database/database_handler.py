...
class DatabaseHandler:
    ...
    def _get_connection(self):
        print("Attempting to connect to DB...")  # 함수 호출 로그
        return sqlite3.connect(self.db_path)

    def _initialize_attendance_db(self):
        print("Initializing attendance DB...")
        conn = sqlite3.connect(self.db_path)
        if conn is None:
            print("Failed to get DB connection during initialization.")
        ...
        try:
            ...
            ''')
            conn.commit()
            print("Attendance DB initialized successfully.")
        except sqlite3.Error as e:
            print(f"Error initializing attendance DB: {e}")
        finally:
            conn.close()
